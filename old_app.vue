  ...
  <div>
    <nav class="absolute top-0 z-10 w-full bg-slate-800 bg-opacity-80">
      <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <button id="hideShow"
          class="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none font-bold rounded-lg text-sm px-5 py-2.5 me-2 bg-slate-800 dark:hover:bg-gray-700">Weltraumschrott
          anzeigen</button>
...
interface SatelliteData {
  satrec: any; // Define the correct type for satrec if available
  name: string;
  lat?: number;
  lng?: number;
  alt?: number;
...
const initGlobe = () => {
  world.value = Globe({ waitForGlobeReady: true, animateIn: false })(document.getElementById('chart'))
  ...
  getUserPosition()
  addMoon()
  setInterval(updateISSPosition, 4000);
};
...
const updateISSPosition = () => {
  fetch('http://api.open-notify.org/iss-now.json')
    ...
    .then(data => {
      ...
      const issMesh = world.value.scene().getObjectByName('issMesh');

      if (issMesh) {
        // Die ISS existiert bereits, aktualisieren Sie ihre Position
        issMesh.position.copy(issPosition);
      } else {
        // Die ISS existiert noch nicht, erstellen Sie sie
        const ISS_RADIUS = 2; // Radius der ISS-Kugel
        const issMaterial = new THREE.MeshPhongMaterial({ color: 0xc0c0c0 });
        const issGeometry = new THREE.CapsuleGeometry(ISS_RADIUS, 4, 3, 10);
        const newIssMesh = new THREE.Mesh(issGeometry, issMaterial);
        newIssMesh.position.copy(issPosition);
        newIssMesh.name = 'issMesh'; // Geben Sie der ISS einen Namen, um sie später identifizieren zu können
        world.value.scene().add(newIssMesh);
      }
